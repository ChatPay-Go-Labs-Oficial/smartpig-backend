...
export class EtherfuseUpstreamError extends HttpException {
  constructor(statusCode: number, message: string) {
    super(`Etherfuse upstream error (${statusCode}): ${message}`, HttpStatus.BAD_GATEWAY);
  }
}
...
export function mapEtherfuseError(error: unknown): never {
  ...
  if (error && typeof error === 'object') {
    ...
    if (typeof plain['statusCode'] === 'number') {
      ...
      throw new EtherfuseUpstreamError(plain['statusCode'] as number, message);
    }
  }

  if (error instanceof Error) {
    logger.error(`Etherfuse unexpected error: ${error.message}`);
    throw new HttpException(`Etherfuse error: ${error.message}`, HttpStatus.BAD_GATEWAY);
  }
